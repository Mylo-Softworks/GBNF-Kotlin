...
import com.mylosoftworks.gbnfkotlin.sanitizeGBNFRangeChars

class GBNFRangeRule(val rangeDef: String, val negate: Boolean = false): GBNFRule() {
    override fun compile(): String {
        return "[" + (if (negate) "^" else "") +
                "${sanitizeGBNFRangeChars(rangeDef)}]" // Prevent mistakes when the user already escapes
    }
    ...
    override fun parse(string: String): Result<Pair<ParseResult<*>, String>> {
        ...
        if (validateCharacter(char)) {
            return Result.success(ParseResult(char.toString(), this) to string.substring(1))
        }
